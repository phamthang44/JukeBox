...
with open(r'D:\Desktop\GCS230575 - Python OOP\Coursework-main\JukeBox\Main\data.csv', mode="r", encoding='utf-8') as file:
    ...
    for row in reader:
        if len(row) < 5:  # ensure add full value
        ...
        youtube = row[4]
        duration = int(row[5])
        library[track_id] = LibraryItem(track_name, artist, rating, youtube_link=youtube, duration=duration)

# test for print 
# for key, item in library.items():
#     print(f"{key}: {item.info()}, duration : {item.duration}")
